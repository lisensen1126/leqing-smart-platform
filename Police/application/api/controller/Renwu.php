...
class Renwu extends Api
{
    ...
    //  var url = "/Api/Renwu/Add?"+"&renwu_id="+{$renwu_id} +"&userids="+userids;
    public function Add(){
        ...
        for($i=0;$i<count($useridlist)-1;$i++){
            ...
            $Renwu->renwu_id = $_GET["renwu_id"];
            $Renwu->user_id = $useridlist[$i];
            $res = $Renwu->where(['renwu_id'=>$_GET["renwu_id"],'user_id'=>$useridlist[$i]])->select();
            if($res){
                // echo "cunzai";
            }else{
                $res = $Renwu->save(); 
