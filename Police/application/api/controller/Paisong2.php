...
class Paisong2 extends Api
{
    ...
    //  var url = "/Api/Paisong2/Add?"+"&renwu_id="+{$renwu_id} +"&userids="+userids;
    public function Add(){
        ...
        for($i=0;$i<count($useridlist)-1;$i++){
            $paisong = new \app\admin\model\Paisong2;
            $paisong->renwu_id = $_GET["renwu_id"];
            $paisong->user_id = $useridlist[$i];
            $res = $paisong->where(['renwu_id'=>$_GET["renwu_id"],'user_id'=>$useridlist[$i]])->select();
            if($res){
                // echo "cunzai";
            }else{
                $res = $paisong->save(); 
