...
class Paisong extends Api
{
    ...
    //
    // 无需登录的接口,*表示全部
    protected $noNeedLogin = ['index', 'test1','File','Add','Find','Jingqing','Jiangli'];
    // 无需鉴权的接口,*表示全部
    ...
    public function Jiangli(){
        ...
        // $data['paisongid'] = $_GET['paisongid'];
        $data['result'] = $_GET['result'];
        $data['state'] = "已完结";
        $data['score'] = $_GET['score'];
        $res=$Paisong->save($data,['id' => $_GET['paisongid']]);


        $respaisong=$Paisong->where('id='. $_GET['paisongid'])->select(); 
        $User = new \app\admin\model\User;
        $res=$User->where('id='.$respaisong[0]['user_id'])->setInc('score',$_GET['score']); // 用户的积分加3
        // $User->save(); 
        // $user = $User::get($respaisong[0]['user_id']);
        // $user->score     = $score->name+$_GET['score'];
        // $res=$User->save();
    ...
    //增加http://127.0.0.1:84/Api/Paisong/Add?&jingqing_id=140&userids=1,2,3,
    public function Add(){

        $jingqing = new \app\admin\model\Jingqing;
        $jingqing->save(['state'=>'已派送'],['id'=>$_GET["jingqing_id"]]); 
        ...
        for($i=0;$i<count($useridlist)-1;$i++){
            $paisong = new \app\admin\model\Paisong;
            $paisong->jingqing_id = $_GET["jingqing_id"];
            $paisong->user_id = $useridlist[$i];
            $res = $paisong->where(['jingqing_id'=>$_GET["jingqing_id"],'user_id'=>$useridlist[$i]])->select();
            if($res){
                // echo "cunzai";
            }else{
                $res = $paisong->save(); 
