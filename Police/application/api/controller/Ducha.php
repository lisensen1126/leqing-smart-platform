...
class Ducha extends Api
{
    ...
    //  var url = "/Api/Ducha/Add?"+"&renwu_id="+{$renwu_id} +"&userids="+userids;
    public function Add(){
        ...
        for($i=0;$i<count($useridlist)-1;$i++){
            ...
            $Ducha->renwu_id = $_GET["renwu_id"];
            $Ducha->user_id = $useridlist[$i];
            $res = $Ducha->where(['renwu_id'=>$_GET["renwu_id"],'user_id'=>$useridlist[$i]])->select();
            if($res){
                // echo "cunzai";
            }else{
                $res = $Ducha->save(); 
